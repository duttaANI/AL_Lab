...
public:
	...
	int BinSearch(vector<int>& X, vector<int>& Y, int* start, int* end, int* partitionX, int* partitionY, int* found, int tot_len)
	{
		...
		int maxLeftX = ((*partitionX-1)==0 )? int_MIN : X[*partitionX-1];
		int minRightX = ((*partitionX)== X.size() )? int_MAX : X[*partitionX];

		int maxLeftY = ((*partitionY-1)==0 )? int_MIN : Y[*partitionY-1];
		int minRightY = ((*partitionY)== Y.size() )? int_MAX : Y[*partitionY];
    ...
    double findMedianSortedArrays(vector<int>& nums1, vector<int>& nums2) {
        ...
        if( tot_len%2 == 0 ) 
        {
        	maxLeftX = ((partitionX-1)==0 )? int_MIN : nums1[partitionX-1];
			minRightX = ((partitionX)== nums1.size() )? int_MAX : nums1[partitionX];

			maxLeftY = ((partitionY-1)==0 )? int_MIN : nums2[partitionY-1];
			minRightY = ((partitionY)== nums2.size() )? int_MAX : nums2[partitionY];
...
public:
	...
	int BinSearch(vector<int>& X, vector<int>& Y, int* start, int* end, int* partitionX, int* partitionY, int* found, int tot_len)
	{
		...
		else if ( (X[(*partitionX)-1]>Y[(*partitionY)]) )  // move towards left in X
		{
			*start = (*partitionX) + 1;
			*partitionX = ((*start)+(*end))/2;
			*partitionY = (tot_len+1)/2 - (*partitionX);
    ...
    double findMedianSortedArrays(vector<int>& nums1, vector<int>& nums2) {
        ...
        int len1 = nums1.size();    int len2 = nums2.size();    int tot_len = len1+len2;
        vector<int>& X = (len1<=len2)? nums1 : nums2; 
        vector<int>& Y = (len1> len2)? nums1 : nums2;
        int start = 0;              int end = X.size();  int partitionX = (start+end)/2;   int partitionY = (tot_len+1)/2 - partitionX;
        int found = 0;    
        int val;

        while(found!=1)
    	{
    		val = BinSearch(X, Y, &start, &end, &partitionX, &partitionY, &found, tot_len);
    	}

        if( tot_len%2 == 0 ) 
        {
        	return (double)(max(X[partitionX-1],Y[partitionY-1])+min(X[partitionX],Y[partitionY]))/2.0;
        }
        else
        {
        	return (double)max(X[partitionX-1],Y[partitionY-1]);
        }
